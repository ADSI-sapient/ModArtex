...
function PedidoAsociado(clienteReg){
  $.ajax({
  type: 'post',
  dataType: 'json',
  url: uri+"ctrCostizacion/fichaAsociada",
  data:{idCli: clienteReg}
  }).done(function(respuesta){
  if (respuesta.r != null) {
  $("#Aso-pedido > tbody tr").empty();
  arrayProductos = respuesta.r;
  for (var i = 0; i <= arrayProductos.length - 1; i++) {
  idProducto = arrayProductos[i]['Id_Solicitudes_Producto'];
  color = arrayProductos[i]['Id_Color'];
...
function Cambiar_Producto(btn, elemento, subtotal){
  var e = $(elemento).parent().parent();
  $(e).remove();
  boton = "#btn"+btn;
  $(boton).attr('disabled', false);
...
function Modificar_ProductoAso(referencia, color, vlrproducto, productos, idbton){
  ...
  var tr = "<tr class='box box-solid collapsed-box'><td id=''>"+referencia+"</td><td><i class='fa fa-square' style='color: "+color+"; font-size: 150%;'></i></td><td><input type='number' min='1' id='cantProducir"+referencia+"' name='cantProducir[]' value='0' onchange='res"+referencia+".value=cantProducir"+referencia+".value * "+vlrproducto+"; subt"+referencia+".value=parseFloat(res"+referencia+".value); total_Pedido();' style='border-radius:5px;'></td><td>$"+vlrproducto+"</td><td><input class='subtotal' type='hidden' name='subtotal[]' id='subt"+referencia+"'value='0'><input readonly='' type='text' id='capValor"+referencia+"' name='res"+referencia+"' for='cantProducir"+referencia+"' style='border-radius:5px;'></td><td><button type='button' onclick='Cambiar_Producto("+referencia+", this, subt"+referencia+".value)' class='btn btn-box-tool'><i class='fa fa-remove'></i></button></td><input type='hidden' id='idProducto"+referencia+"' name='idProducto[]' value="+referencia+"></tr>";
...
function RegiCotizacion(){

swal({   title: "Desea Registrar La Cotizacion",   
  text: "La Cotizacion Pasara a Listar",   
  type: "info",   
